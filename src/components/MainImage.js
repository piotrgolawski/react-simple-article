...
function MainImage(props) {
    ...
    if(get(props, 'value.leadImage.url')) {
        URI = `https://my12.digitalexperience.ibm.com/${get(
            props,
            ['value', 'leadImage', 'renditions', getProperImage() , 'url']
        )}`;
    }
    ...
    if(URI) {
        return (
            <CardMedia
                ...
                image={URI}
                title={get(props, 'value.leadImageCaption.value')}
            />
        );
    } else return (<span/>);
...
function mapStateToProps(state) {
    if (state.article && state.article.mainImage ) {
        return { ...state.article.mainImage };
    } else {
        return { }
    }
